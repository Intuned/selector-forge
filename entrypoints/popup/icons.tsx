import type { SVGProps } from "react";

export {
  Target as TargetIcon,
  List as ListIcon,
  ChevronDown,
  MousePointerClick as CursorClick,
  Code2 as CodeBrackets,
  Copy as CopyIcon,
  Check as CheckIcon,
  RotateCcw as UndoIcon,
  LogOut as SignOutIcon,
  LogIn as SignInIcon,
  KeyRound as KeyIcon,
  Crosshair as LocateIcon,
  SquareDashedMousePointer as NewSelectorIcon,
  ThumbsUp as ThumbsUpIcon,
  ThumbsDown as ThumbsDownIcon,
  Settings as SettingsIcon,
  TriangleAlert as AlertIcon,
} from "lucide-react";

/** Selector Forge mark — lime tile with the near-black forge glyph. */
export function ForgeLogo(props: SVGProps<SVGSVGElement>) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      role="img"
      aria-label="Selector Forge"
      {...props}
    >
      <rect width="24" height="24" fill="#C8FF2E" />
      <g fill="#0B0B0B" transform="translate(2 6.3056) scale(0.277778)">
        <path d="M1.4788e-06 3.31909H18.8934L18.8913 15.1392L13.6938 15.1442C12.6571 15.1447 11.304 15.1821 10.2979 15.0935C7.68967 14.8622 5.24424 13.7268 3.38445 11.8835C0.950513 9.48717 0.0160763 6.66674 1.4788e-06 3.31909Z" />
        <path d="M65.7514 0.0140934C67.577 0.0231792 69.4024 0.0185198 71.2279 -1.19442e-06C71.2262 1.03916 71.2167 2.09043 71.2284 3.12842C64.0971 3.86367 59.8285 8.2592 58.4106 15.1414C55.9854 15.1342 53.3695 15.1855 50.9627 15.1303C47.5129 15.1971 43.8809 15.1414 40.4106 15.1409L20.5312 15.1381C20.5774 13.3623 20.5473 11.4737 20.5473 9.69067L20.5458 0.00197904C25.8852 0.0629003 31.3093 0.0153747 36.6537 0.0153747L65.7514 0.0140934Z" />
        <path d="M23.9733 16.8621C24.4506 16.7846 26.3549 16.8231 26.9644 16.8236L33.2499 16.8264L46.1653 16.8248C48.4668 16.8248 51.0709 16.7679 53.3468 16.856C50.9009 18.973 49.2917 21.2583 48.9923 24.5782C48.6619 28.2369 50.7209 32.1197 53.5965 34.3098C51.1311 34.3152 48.465 34.366 46.0148 34.3114C45.8224 34.3264 45.8113 34.3281 45.6213 34.3008C45.5025 34.1776 45.2377 33.4352 45.1056 33.1843C44.157 31.3823 42.5829 30.0796 40.6152 29.515C38.7489 28.986 36.7488 29.2202 35.0552 30.1661C33.4638 31.0551 32.1751 32.5556 31.671 34.3192C29.004 34.2958 26.332 34.337 23.6616 34.3036C29.5647 29.4564 29.9295 21.8731 23.9581 16.9268L23.9733 16.8621Z" />
        <path d="M15.3425 40.5805V35.991H33.0482C33.0482 32.9674 35.4993 30.5162 38.5229 30.5162C41.5466 30.5162 43.9977 32.9674 43.9977 35.991H61.9363V40.5805H15.3425Z" />
      </g>
    </svg>
  );
}
